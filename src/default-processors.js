module.exports = function(constants, systemEntities) {
	...
	let createProcessCode = (() => {
		...
		listEntityTemplate = (() => {
			...
			if (this.args && this.args.count) {
				...
				if (this.args.query) _.assign(query, this.libs.convertFilter(this.args.query));
			}
		...
		createSchemaCode = (() => {
			function resolve(type, data) {
				...
				function parsePropertyWithoutName(template, x) {
					if (x.propertyType !== constants.ENTITYTYPE.REFERENCE) throw new Error("all entity types must have a propertyName except REFERENCE");
	...
	return createProcessor
		.call(
			...
			listEntityTemplate.replace("$entity", "args.entityName").replace("$get", "get").replace("$parameters", "entity,query").replace("$label", "[args.entityLabel]"),
			constants.UIDS.PROCESSOR.LIST_ENTITY_GENERIC
		)
		.createProcessor(
			"Lists processors",
			listEntityTemplate.replace("$entity", `'${systemEntities.processor}'`).replace("$get", "getProcessor").replace("$parameters", "query").replace("$label", ".title"),
		...
		.createProcessor(
			"Lists processes",
			listEntityTemplate.replace("$entity", `'${systemEntities.process}'`).replace("$get", "getProcess").replace("$parameters", "query").replace("$label", ".title"),
			constants.UIDS.PROCESSOR.LIST_PROCESSES
		)
		.createProcessor(
			"Lists libs",
			listEntityTemplate.replace("$entity", `'${systemEntities.lib}'`).replace("$get", "getLib").replace("$parameters", "query").replace("$label", ".uid"),
		...
		.createProcessor("Create or Edit Processor", createProcessCode, constants.UIDS.PROCESSOR.CREATE_PROCESSOR)
		.createProcessor("Create or Edit Lib", createLibCode, constants.UIDS.PROCESSOR.CREATE_LIB)
		...
		.createProcessor("Update Schema", updateSchemaCode, constants.UIDS.PROCESSOR.UPDATE_SCHEMA).processors;
};
