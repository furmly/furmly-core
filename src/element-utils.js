...
module.exports = {
  ...
  convert: function(factory, parent, name, extensionParams) {
    if (parent && parent[name] && parent[name].length) {
      parent[name] = factory.getAll(factory, parent[name], extensionParams);
    }
  },
  ...
  elementInvariants: {
    _ensureArgs: function(item) {
      if (!item.args)
        throw new Error(`element ${JSON.stringify(item)} must contain args`);
    },
